...
import Add_Author from '../../Component/Modals/Add_Author';
import { Link } from 'react-router-dom';
import Update_Author  from './Update_Author';
...
const Author = () => {
  ...
  useEffect(() => {
    const fetchData = async (e) => {
      ...
      await axios.get(`http://127.0.0.1:8000/api/Authors`)
        .then((response) => {
          if (response.status === 200) {
            console.log((response.data))

            setAuthors([...response.data.authors])
            setFilteredData([...response.data.authors])
  ...
  const deleteFunction = async (e) => {
    ...
    try {
      const response = await axios.delete('http://127.0.0.1:8000/api/authors/delete/' + e.target.value);
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 ">
              <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                  ...
                  <th scope="col" class="px-4 py-3">day of death</th>
                </tr>
              </thead>
              <tbody >

                {
                  filteredData == null ? (
                    ...
                    records.map((author, index) => {
                      return (<tr key={author.id} class="border-b dark:border-gray-700  w-full">
                        <td class="px-4 py-3">

                          <div className='flex w-full h-full'>

                            <img className='h-8 w-8 object-cover rounded-full' src={`http://127.0.0.1:8000/${author.image}`} alt='' />

                          </div>
                        </td>
                        <Link to={`/Authors/${author.id}`} scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white hover:underline point-cursre">{author.name}</Link>
                        <td class="px-4 py-3">{author.birthday}</td>

                        <td class="px-4 py-3 ">{author.daye_death}</td>

                        <td class="px-4 py-3 w-32 flex items-center   ">





                          <Link to={`/Authors/${author.id}`} class="flex me-  items-center   border border-blue-600 p-1 rounded-md  hover:bg-blue-600 hover:text-white text-blue-700 ">
                          ...
                          <button onClick={(e) => deleteFunction(e)} value={author.id} type="button" class="flex items-center  border border-red-600 p-1 rounded-md hover:bg-red-600 ms-1 text-red-500 hover:text-white">

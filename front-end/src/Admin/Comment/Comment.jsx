...
const Comment = () => {
    ...
    const deleteFunction=async(e)=>{
        ...
        try {
            ...
            toast.success('you delete comment',{icon:''})
            setcount(!count)
    ...
    return (
        <section class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5 w-full pt-2 pb-2 ">
    ...
    <div className='mt-2 mb-2'>
                                {
                                    filteredData == null ? (
                                        ...
                                        records.map((comment) => {
                                            const created_at = moment(comment.created_at).fromNow()

                                            return (                    <article class="p-6 text-warp bg-white  border-b-2 border-blue-700  dark:bg-gray-900">
                                            <footer class="flex justify-between  mb-2">
                                                ...
                                                <p className='text-sm text-gray-600 dark:text-gray-400'>{comment.user.email}</p>
                    
                                            </footer>
                                            <p className='text-gray-500 dark:text-gray-400 text-ellipsis overflow-hidden'>{comment.content}</p>
                                            <p className='text-xs text-gray-400' >Commented on : <Link to={`/Books/${comment.book.id}`} className='truncate font-medium text-indigo-600'>{comment.book.title}</Link></p>
                                            <div class="flex justify-between mt-4 space-x-4">
                                                ...
                                                <button onClick={(e) => deleteFunction(e)} value={comment.id} type="button" class="flex items-center  border border-red-600 p-1 rounded-md hover:bg-red-600 ms-1 text-red-500 hover:text-white">

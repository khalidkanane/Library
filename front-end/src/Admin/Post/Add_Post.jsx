...
const Add_P = () => {
  ...
  const generateTextFromGemini = async () => {
    if (!editorHtml || editorHtml.trim().length < 3) {
      toast.error('Please write some prompt text first');
      return;
    }

    try {
      const response = await axios.post('http://127.0.0.1:8000/api/generate-text', {
        prompt: editorHtml,
      });

      const generatedText = response?.data?.text || '';
      if (!generatedText) {
        toast.error('No text returned from AI');
        return;
      }
